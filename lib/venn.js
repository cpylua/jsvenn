...
(function() {
  var calcAngles, calcRadiuses, calcRadiuses3, draw2, draw3, findAngle, findDistance, findDistance3, g_fontFamily, g_fontSize, g_margin, g_threshold, intersectionArea, isFiniteNumber, isNumber, isValidAngles, normalizeWeights, normalizeWeights3, root, scaleIntersection, scaleIntersection3, venn2, venn3;
  ...
  venn2 = function(container, weights, baseRadius, opts) {
    var distance, height, interArea, labels, paper, radiuses, width, _ref;
    if (baseRadius == null) {
      baseRadius = 100;
    }
    opts = opts || {
    ...
    return draw2(paper, height, radiuses, distance, opts, labels);
  };
  ...
  */
  ...
  normalizeWeights = function(weights) {
    var idx, item, maxOverlap, _i, _len, _ref;
    _ref = weights.cards;
  ...
  draw2 = function(paper, height, radiuses, distance, opts, labels) {
    var bb, bbs, cxleft, cxright, cyleft, cyright, left, r1, r2, right, rightEdges, t, t1, t2, texts, tx, ty1, ty2, w, _i, _len, _ref, _ref1;
    ...
    w = Math.max.apply(Math, (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = bbs.length; _i < _len; _i++) {
        bb = bbs[_i];
        _results.push(bb.x2);
      }
      return _results;
  ...
  */
  ...
  normalizeWeights3 = function(weights) {
    var a, ab, abMin, ac, acMin, b, bc, bcMin, c, idx, item, _i, _len, _ref, _ref1, _ref2;
    _ref = weights.cards;
    for (idx = _i = 0, _len = _ref.length; _i < _len; idx = ++_i) {
      item = _ref[idx];
      if (item < 0) {
    ...
    _ref1 = weights.overlap, ab = _ref1[0], ac = _ref1[1], bc = _ref1[2];
    _ref2 = weights.cards, a = _ref2[0], b = _ref2[1], c = _ref2[2];
    abMin = Math.min(a, b);
    acMin = Math.min(a, c);
    bcMin = Math.min(b, c);
  ...
  scaleIntersection3 = function(overlap, maxRadius, maxWeight) {
    var scale, weight, _i, _len, _results;
    scale = function(weight) {
      if (maxWeight === 0) {
        return 0;
      } else {
        return Math.PI * Math.pow(maxRadius, 2) * weight / maxWeight;
      }
    };
    _results = [];
    for (_i = 0, _len = overlap.length; _i < _len; _i++) {
      weight = overlap[_i];
      _results.push(scale(weight));
    }
    return _results;
  };
